    ...
    try {
        ...
        if (!apiKey) {
            return NextResponse.json({
                reply:
                    "Error: GEMINI_API_KEY is missing from your Vercel Environment Variables. Please add it in your Vercel dashboard!",
            });
        }
        ...
        if (fileData && mimeType) {
            ...
            let instruction =
                "Provide a comprehensive, professional summary of this document for a government statistical officer under MoSPI.";

            if (mode === "quiz") {
                instruction =
                    "Generate 1 clear multiple-choice question with 3 options based on this document. Format: Question, then Options A/B/C, then the correct answer.";
            } else if (prompt) {
                instruction = prompt;
            }
            ...
            if (sourceContent || mode) {
                ...
                if (mode === "summary") {
                    ...
                    promptText = `You are an expert quiz generator. Generate a clear multiple-choice question with 3 options based on this source text. Format: Question, then Options A/B/C, then the correct answer.\n\nSource:\n${sourceContent}`;
                } else {
